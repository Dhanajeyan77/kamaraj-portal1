...
def execute_user_code(user_code, expected_answer, queue, lang='python'):
    ...
    # --- JAVA EXECUTION ---
        ...
        try:
            ...
            # 1. Compile with a generous timeout for slow Free Tier CPUs
            ...
            if compile_process.returncode != 0:
                ...
                # 2. Execute with small memory footprint (-Xmx128m) to avoid crashes
            ...
            result['error'] = "Timeout Error: Java took too long to compile or run!"
        except Exception as e:
...
def question_detail(qid):
    ...
    if request.method == "POST":
        ...
        p.start()
        # Wait up to 20 seconds total for the whole process to complete
        p.join(timeout=20) 

        if p.is_alive():
            p.terminate()
            output = "❌ Timeout Error: Server is too busy to run this code."
        else:
            if not queue.empty():
                res = queue.get()
                if res.get('error'):
                ...
                output = "⚠️ Error: Execution failed to return a result."
